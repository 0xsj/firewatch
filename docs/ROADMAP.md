...
### Testing
- [x] 13 test suites, 76+ tests across all layers
...
- Useful for detecting coordinated scanning vs single-source brute force
- `internal/middleware/ratelimit.go`

### External Signature Files
- Load signatures/patterns from YAML files instead of (or in addition to) Go code
- `signatures/scanners.yaml`, `signatures/patterns.yaml`
...
### JA4 Fingerprinting
- JA4 extends JA3 with richer TLS metadata
- `internal/fingerprint/ja4.go` — not yet implemented
- Config already has `fingerprinting.ja4` toggle

### Behavioral Fingerprinting
- Track request sequences per-IP to identify scanner behavior patterns
- Time between requests, path traversal order, retry patterns
